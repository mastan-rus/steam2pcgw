...
func main() {
	...
	for _, v := range game.Data.Demos {
		dlcs += fmt.Sprintf("%v, ", v.AppID)
	}
	...
	output = strings.ReplaceAll(output, "$demo_info$", demo_info)
	...
	sort.Slice(orderedLangauges, func(i, j int) bool {
		s1 := orderedLangauges[i]
		s2 := orderedLangauges[j]
